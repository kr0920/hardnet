...
if __name__ == '__main__':
    ...
    try:
          ...
          print("Wrong input format. Try ./extract_hardnet_desc_from_hpatches_file.py /home/mitc/3dconstruction/ORB_SLAM2-master/Examples/rgbd_dataset_freiburg1_xyz/rgb gpu")
          sys.exit(1)
    #model_weights = '../pretrained/train_liberty_with_aug/checkpoint_liberty_with_aug.pth'
    #model_weights = '../code/data/models/margin_03siftliberty_train_03sift/_liberty_min_as_fliprot/checkpoint_9.pth'
    ...
    if DO_CUDA:
        ...
        model = model.cpu()
	#pdb.set_trace()
    for input_img_fname in os.listdir(input_img):
        for filename in os.listdir(input_img+'/'+input_img_fname):
            if os.path.splitext(filename)[1]=='.png':
                liststr=['../output_hpatches_margin/',input_img_fname,'/',filename[:-4],'.csv']
                output_filename=''.join(liststr)
                img_dir=input_img+'/'+input_img_fname+'/'+filename
                print(img_dir)
                ...
                np.savetxt(''.join(liststr), descriptors_for_net, delimiter=',', fmt='%10.5f')
