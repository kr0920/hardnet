...
def loss_HardNet(anchor, positive, out_sift,out_sift_p,anchor_swap = False, anchor_ave = False,\
        margin = 1.15, batch_reduce = 'min', loss_type = "triplet_margin"):
    ...
    #print("dist1 ok")
    #print("aaaaaaaa",anchor.type,"ssssssift",Variable(out_sift).type)
    dist_matrix1 = distance_matrix_vector(Variable(out_sift).cuda(), Variable(out_sift_p).cuda()) +eps
    #print("disk2 ok")
    eye = torch.autograd.Variable(torch.eye(dist_matrix.size(1))).cuda()

    # steps to filter out same patches that occur in distance matrix as negatives
    pos1 = torch.diag(dist_matrix)
    pos2 = torch.diag(dist_matrix1)
    dist_without_min_on_diag = dist_matrix+eye*10
    ...
    if batch_reduce == 'min':
        ...
        sys.exit(1)
    if loss_type == "triplet_margin":
        #loss = torch.clamp(5*pos2 + pos - min_neg, min=0.0)
        loss=torch.clamp(margin + pos + 0.3*pos2 - min_neg, min=0.0)
    elif loss_type == 'softmax':
        exp_pos = torch.exp(2.0 - pos);
        exp_den = exp_pos + torch.exp(2.0 - min_neg) + eps;
        loss = - torch.log( exp_pos / exp_den )
