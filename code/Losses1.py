...
def loss_HardNet(anchor, positive, out_sift,anchor_swap = False, anchor_ave = False,\
    ...
    dist_matrix1 = distance_matrix_vector(anchor, Variable(out_sift).cuda()) +eps
    #print("disk2 ok")
    eye = torch.autograd.Variable(torch.eye(dist_matrix.size(1))).cuda()
    ...
    if batch_reduce == 'min':
        ...
        sys.exit(1)
    if loss_type == "triplet_margin":
        loss = torch.clamp(margin+0.3*pos2 + pos - min_neg, min=0.0)
    elif loss_type == 'softmax':
        exp_pos = torch.exp(2.0 - pos);
        exp_den = exp_pos + torch.exp(2.0 - min_neg) + eps;
        loss = - torch.log( exp_pos / exp_den )
