...
from Losses_sift import loss_HardNet, loss_random_sampling, loss_L2Net, global_orthogonal_regularization
...
parser.add_argument('--log-dir', default='data/logs/margin_03sift',
                    help='folder to output log')
parser.add_argument('--model-dir', default='data/models/margin_03sift',
                    help='folder to output model checkpoints')
parser.add_argument('--experiment-name', default= 'liberty_train_03sift/',
...
def siftCal(img):
    #print(img)
    img_re=img.reshape([32,32])
    #print(type(np.array(img)))
    img_cal=img_as_ubyte(img_re)
    siftDetector=cv2.xfeatures2d.SIFT_create()
    step_size=32
    #print(img_cal.shape)
    kp=[cv2.KeyPoint(x,y,step_size) for y in range(0,img_cal.shape[0],step_size)
                                    for x in range(1,img_cal.shape[1],step_size)]
    #print("kp",kp)
    kp,des=siftDetector.compute(img_cal,kp)
...
def train(train_loader, model, optimizer, epoch, logger, load_triplets  = False):
    ...
    model.train()
    transform_data=transforms.Compose([transforms.Normalize(args.mean_image,args.std_image)])
    pbar = tqdm(enumerate(train_loader))
    for batch_idx, data in pbar:
        if load_triplets:
            data_model=transforms_data(data.numpy())
            data_a, data_p, data_n = data_model
        else:
            data_a, data_p = data

        data_sift=data_a[0]
        data_sift_p=data_p[0]
        #print(type(data_sift))
        #print(data_sift)
        #print(data_sift[0].numpy())
        #print(cv2.__version__)
        out_sift1=siftCal(data_sift[0].numpy())
        out_sift1_p=siftCal(data_sift_p[0].numpy())
        #print(np.array(data_sift[0].shape))
        #print(out_sift1)
        data_sift=data_a[1]
        data_sift_p=data_p[1]
        out_sift2=siftCal(data_sift.numpy())
        out_sift2_p=siftCal(data_sift_p[0].numpy())
        out_sift=np.vstack((out_sift1,out_sift2))
        out_sift_p=np.vstack((out_sift1_p,out_sift2_p))
        #print("out_sift",out_sift)
        #print(len(data_a.numpy()))
        for i in range(2,len(data_a.numpy())):
            data_sift=data_a[i]
            data_sift_p=data_p[i]
            out_sift3=siftCal(data_sift.numpy())
            out_sift4=siftCal(data_sift_p.numpy())

            out_sift=np.vstack((out_sift,out_sift3))
            out_sift_p=np.vstack((out_sift_p,out_sift4))
        #print(out_sift)
        out_sift=torch.from_numpy(out_sift)
        out_sift_p=torch.from_numpy(out_sift_p)

        if args.cuda:
            #data_a, data_p  = data_a.cuda(), data_p.cuda()
            data_a_normalize=data_a.numpy()*args.std_image+args.mean_image
            data_p_normalize=data_p.numpy()*args.std_image+args.mean_image
            data_a=torch.from_numpy(data_a_normalize)
            data_p=torch.from_numpy(data_p_normalize)
            data_a, data_p = data_a.cuda(),data_p.cuda()
        ...
        if args.batch_reduce == 'L2Net':
            ...
            loss = loss_HardNet(out_a, out_p,out_sift,out_sift_p,
                            margin=args.margin,
                            anchor_swap=args.anchorswap,
